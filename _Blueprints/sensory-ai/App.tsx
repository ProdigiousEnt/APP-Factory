...
const App: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const script = await geminiService.generateMeditationScript(theme);
      updateStepStatus(0, 'completed');

      updateStepStatus(1, 'loading');
      const imageUrl = await geminiService.generateMeditationImage(theme);
      updateStepStatus(1, 'completed');

      updateStepStatus(2, 'loading');
      const audioData = await geminiService.generateTTS(script);
      ...
      const newSession: MeditationSession = {
        ...
        script,
        imageUrl,
        audioUrl: audioData, // audioData from TTS is the URL/data string
      ...
      setError("Unable to generate session. Please check your connection and API key.");
    } finally {
        ...
        <div className="bg-slate-900/40 border border-slate-800 p-6 rounded-3xl shadow-xl backdrop-blur-sm space-y-6">
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              <h4 className="text-[10px] font-bold text-slate-600 uppercase tracking-widest px-1">Choose Your Guide</h4>
